...
try { execSync(`open "${authUrl}"`); } catch { /* browser open failed, user will copy URL */ }
...
const code = await new Promise<string>((resolve, reject) => {
  ...
  setTimeout(() => { server.close(); reject(new Error("Timed out waiting for authorization")); }, 120_000);
});
